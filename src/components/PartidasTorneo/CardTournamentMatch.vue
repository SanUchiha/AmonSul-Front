<template>
  <div>
    <v-card :class="cardColorClass" @click="goToDetallePartida()">
      <v-card-title>
        <strong>{{ match.nickUsuario1 }} - {{ match.nickUsuario2 }}</strong>
      </v-card-title>

      <v-card-text>
        <p>
          <strong>Resultado:</strong> {{ match.resultadoUsuario1 }} -
          {{ match.resultadoUsuario2 }}
        </p>
      </v-card-text>

      <v-card-text v-if="match.puntosPartida">
        <p><strong>Puntos:</strong> {{ match.puntosPartida }} puntos</p>
      </v-card-text>

      <v-card-text>
        <p>
          <strong>Escenario:</strong>
          {{ match.escenarioPartida || "No disponible" }}
        </p>
      </v-card-text>

      <v-card-text>
        <p><strong>Fecha:</strong> {{ fechaPartidaFormateada }}</p>
      </v-card-text>

      <div class="resultado">{{ resultado }}</div>
    </v-card>
    <v-spacer class="my-3"></v-spacer>
  </div>

  <ModalDetallePartida
    v-if="showModalDetallePartida"
    :partida="match"
    @close="closeModalDetallePartida"
  />
</template>

<script setup lang="ts">
import { ViewPartidaTorneoDTO } from "@/interfaces/Partidas";
import { formatFechaSpa } from "@/utils/Fecha";
import { defineProps, ref, onMounted, computed } from "vue";
import ModalDetallePartida from "../PartidaAmistosa/ModalDetallePartida.vue";

const props = defineProps<{
  match: ViewPartidaTorneoDTO;
  idUsuario: number;
}>();

const cardColorClass = computed(() => {
  switch (resultado.value) {
    case "Victoria":
      return "victoria-card";
    case "Derrota":
      return "derrota-card";
    case "Empate":
      return "empate-card";
    default:
      return "";
  }
});
const fechaPartidaFormateada = ref<string>("");
const isLoading = ref(true);
const resultado = ref<string>("");
const showModalDetallePartida = ref(false);

const initializeComponent = async () => {
  try {
    isLoading.value = true;
    setResultado();
    if (props.match.fechaPartida != null)
      fechaPartidaFormateada.value = await formatFechaSpa(
        props.match.fechaPartida
      );
  } catch (err) {
    console.error(err);
  } finally {
    isLoading.value = false;
  }
};

const setResultado = () => {
  if (props.match.ganadorPartidaNick == null) {
    resultado.value = "Empate";
  } else if (props.match.ganadorPartidaTorneo == props.idUsuario) {
    resultado.value = "Victoria";
  } else {
    resultado.value = "Derrota";
  }
};

const goToDetallePartida = () => {
  showModalDetallePartida.value = true;
};

const closeModalDetallePartida = () => {
  showModalDetallePartida.value = false;
};

onMounted(initializeComponent);
</script>

<style scoped>
.victoria-card {
  background-color: rgb(14, 84, 14);
  color: white;
  cursor: pointer;
  border-radius: 8px;
  box-shadow: 0px 2px 10px rgba(0, 0, 0, 0.2);
}

.derrota-card {
  background-color: rgb(106, 12, 12);
  color: white;
  cursor: pointer;
  border-radius: 8px;
  box-shadow: 0px 2px 10px rgba(0, 0, 0, 0.2);
}

.empate-card {
  background-color: rgb(151, 151, 15);
  color: black;
  cursor: pointer;
  border-radius: 8px;
  box-shadow: 0px 2px 10px rgba(0, 0, 0, 0.2);
}

.error {
  color: red;
  text-align: center;
}

.resultado {
  font-weight: bold;
  text-align: center;
}
</style>
